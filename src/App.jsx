      ...
      {canEdit&&(
        <div style={{background:C.offWhite,borderRadius:10,padding:"11px 13px",marginBottom:11,border:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginBottom:9}}>
            {stage.status==="Not Started"&&<Btn onClick={()=>onUpdate({...stage,status:"In Progress",startDate:stage.startDate||nowISO()})} variant="sage" size="sm">Start Stage</Btn>}
            {stage.status!=="Waiting for Client Approval"&&<Btn onClick={()=>onUpdate({...stage,status:"Waiting for Client Approval",approvalRequested:true,approvalRequestDate:nowISO()})} variant="outline" size="sm">Submit for Approval</Btn>}
            <Btn onClick={()=>onUpdate({...stage,status:"Completed",endDate:nowISO()})} variant="primary" size="sm">Mark Completed</Btn>
            <Btn onClick={()=>onUpdate({...stage,status:"On Hold"})} variant="secondary" size="sm">Put On Hold</Btn>
          </div>
...
function ProjectDetail({project:init,user,onUpdate,onBack,allUsers}){
  ...
  const updStage=s=>{
    const currentIndex=project.stages.findIndex(x=>x.id===s.id);
    let stages=project.stages.map(x=>x.id===s.id?s:x);

    if(["Completed","Approved"].includes(s.status)){
      const nextIndex=stages.findIndex((x,i)=>i>currentIndex && x.status==="Not Started");
      if(nextIndex!==-1){
        stages=stages.map((x,i)=>i===nextIndex?{...x,status:"In Progress",startDate:x.startDate||nowISO()}:x);
      }
    }
