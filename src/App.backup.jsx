    ...
    <div style={{minHeight:"100vh",background:C.offWhite,fontFamily:font}}>
      ...
      <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 24px",background:C.white,borderBottom:`1px solid ${C.borderSoft}`,position:"sticky",top:0,zIndex:100}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          ...
          <Btn onClick={onGoLogin} variant="primary" size="sm">Portal →</Btn>
        </div>
      </nav>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"48px 24px 40px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center"}} className="hero-grid">
        <div>
          ...
          <p style={{fontSize:15,color:C.charcoalMid,lineHeight:1.75,margin:"0 0 28px"}}>{company.about.slice(0,160)}…</p>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:32}}>
            <Btn onClick={onGoLogin} variant="primary" size="lg">Client Portal →</Btn>
        ...
        <div className="hero-cards" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {portfolio.slice(0,4).map((item,i)=>{
            ...
            return <div key={item.id} style={{borderRadius:12,overflow:"hidden",boxShadow:`0 4px 14px ${C.shadow}`,transition:"all 0.2s",cursor:"pointer"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-3px)";e.currentTarget.style.boxShadow=`0 10px 28px ${C.shadowMd}`}} onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow=`0 4px 14px ${C.shadow}`}}><div style={{height:i===0?130:100,background:`linear-gradient(135deg,${s}66,${s}33)`,display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="22" height="22" viewBox="0 0 18 18" fill="none" style={{opacity:0.22}}><path d="M9 1L16 5.5V12.5L9 17L2 12.5V5.5L9 1Z" stroke={C.charcoal} strokeWidth="1.2" fill={C.charcoal}/></svg></div><div style={{padding:"9px 11px",background:C.white}}><div style={{fontSize:11,fontWeight:600,color:C.charcoal,fontFamily:fontSerif}}>{item.title}</div><div style={{fontSize:9,color:C.muted,marginTop:1}}>{item.location} · {item.year}</div></div></div>;
          })}
        </div>
      </div>
      ...
      <div id="about" style={{background:C.white,borderTop:`1px solid ${C.borderSoft}`,padding:"56px 24px"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          <div style={{marginTop:36,padding:"20px",background:C.bronzePale,borderRadius:12,border:`1px solid ${C.bronzeLight}`}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{company.cities.map(c=><span key={c} style={{background:C.white,border:`1px solid ${C.bronzeLight}`,color:C.bronzeDark,borderRadius:20,padding:"4px 12px",fontSize:12,fontWeight:600}}>📍 {c}</span>)}</div>
          </div>
        </div>
      </div>
      ...
      <div id="contact" style={{background:C.white,borderTop:`1px solid ${C.borderSoft}`,padding:"56px 24px"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center"}} className="contact-grid">
          ...
          <div style={{background:C.offWhite,borderRadius:14,padding:"28px",border:`1px solid ${C.border}`}}>
            ...
            <Btn onClick={onGoLogin} variant="primary" full size="lg">Register as Client →</Btn>
...
export default function App(){
  ...
  const approveClient=id=>{
    ...
    if(c){
      ...
      toast("Approved",`${c.name} · login: ${c.email} · pass: ${pass}`,"success");
    }
